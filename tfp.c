...
int GetRequestData(char *buff, int len, int *opcode, char *filename, char *username){
	...
	*opcode=ntohs(*((short*)buff));
	if(*opcode!=HEL && *opcode!=DOW && *opcode!=UPL && *opcode!=LDI && *opcode!=DEL){
	...
	//get username
	int i1,j1;
	for (i1=2, j1=0;i1<len || buff[i1]!='\0';++i1, ++j1)
	{
		username[j1]=buff[i1];
		curlen++;
	}
	...
	//get filename
	int i, j;
	for(i=curlen+1, j=0;i<len || buff[i]!='\0';++i, ++j){
		filename[j]=buff[i];
	}
...
void WriteFile(int sock, char *buff, char *dir, char *name, int wr, struct sockaddr_in *addr, socklen_t alen){
	...
	while(next){
		...
		for(i=0;i<5;++i){
			...
			if(ntohs(*((unsigned short*)buff))!=DAT) continue;		
