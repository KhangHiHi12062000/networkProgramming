//
// Created by khangnt-leo on 27/06/2022.
//
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char **argv){
	...
	while(1){
		...
		if(s==-1){
			...
			char cdirectory[NAMELEN];
            strcpy(cdirectory,directory);
			int error;
			int opcode;
			char filename[BUFFLEN-4];
            char username[50];
            if(!(error=GetRequestData(buffer, len, &opcode, filename, username))){
			...
			sprintf(cdirectory+strlen(cdirectory),"/%s\n",username);
            printServerInfo(0,cdirectory);
			if(opcode==HEL) SendHello(csock, buffer, cdirectory, &caddr, alen);
			else if(opcode==DEL) RemoveFile(csock, buffer, cdirectory, filename, write, &caddr, alen);
			else if(opcode==UPL) WriteFile(csock, buffer, cdirectory, filename, write, &caddr, alen);
			else if(opcode==DOW) SendFile(csock, buffer, cdirectory, filename, &caddr, alen);	
			else if(opcode==LDI) SendDir(csock, buffer, cdirectory, filename, &caddr, alen);
